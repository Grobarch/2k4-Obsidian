...
│   ├── classify-tags.mjs          ← promocja gatunków z tags do osobnego pola (kampania+scenariusz); sprząta śmieci/duplikaty case
...
### Klasyfikacja gatunków (rozdzielenie z tags)

```bash
node scripts/classify-tags.mjs                       # dry-run cały vault
node scripts/classify-tags.mjs --apply               # zapis
node scripts/classify-tags.mjs --type scenariusz     # filtr typu
node scripts/classify-tags.mjs --file vault/path.md  # pojedynczy plik
```

Promuje gatunki z `tags:` do osobnego pola `gatunek: [...]` (kampanie + scenariusze). Słownik gatunków: stała `GENRES` w `scripts/schema.mjs` (zamknięta lista, walidator odrzuca nieznane wartości). Sprząta przy okazji placeholdery (`${systemId}`, `templates`, `slug-systemu`, `system`) i duplikaty case (`Wied­min` → `wiedzmin`, `7th-Sea` → `7th-sea`). Tagi rolowe (`bohater-gracza`, `epizod`, `lokacja`, ...) i systemowe są zachowane (w tym warianty jak `l5k1ed` matchowane przez pole `system`). Tagi niejasne (np. `wampir`, `dzikie-pola`) zostają w `tags` i wypisane są w sekcji "manual review".
...
Każdy plik ma YAML frontmatter. Kanoniczne definicje pól → `scripts/schema.mjs` (`TYPE_SCHEMAS`, `SYSTEM_NAMES`, `GENRES`).

**Pole `gatunek`** (kampania, scenariusz): YAML array stringów z zamkniętej listy `GENRES` w `schema.mjs` (`samurajski`, `horror`, `dark-fantasy`, `western`, `szpiegowski`, `steampunk`, `space-fantasy`, `fantasy`, `psychologiczny`, `postapokalipsa`, `apokaliptyczny`, `przygodowy`, `cyberpunk`, `gangsterski`, `science-fiction`, `military`, `grindhouse`, `historyczny`). Walidator (`validate-frontmatter.mjs`) odrzuca wartości spoza listy. Migrację z `tags:` robi `classify-tags.mjs`.
